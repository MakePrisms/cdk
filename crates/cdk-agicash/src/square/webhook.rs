...
impl super::client::Square {
    ...
    pub(crate) async fn setup_webhook_subscription(&self) -> Result<(), Error> {
        ...
        let list_body: Value = self
            .api_client
            .list_webhook_subscriptions(&oauth_token)
            .await?;
            ...
            for subscription in subscriptions {
                ...
                let event_types = subscription
                    ...
                    .map(|events| {
                        events
                            .iter()
                            .any(|e| e.as_str().map(|s| s == "payment.created").unwrap_or(false))
                    })
                    .unwrap_or(false);
        ...
        let response_body: Value = self
            .api_client
            .create_webhook_subscription(&oauth_token, &subscription_json)
            .await?;
