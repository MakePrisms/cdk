...
use super::types::{ListPaymentsParams, PaymentBrand, PaymentData};
use super::util::{
    ...
    pub async fn sync_payments(&self) -> Result<(), Error> {
        ...
        loop {
            ...
            for payment in payments {
                ...
                match Bolt11Invoice::from_str(&bolt11_str) {
                    Ok(invoice) => {
                        ...
                        let key = format!(
                            "{}{}",
                            super::util::INVOICE_HASH_PREFIX,
                            hex::encode(payment_hash)
                        );
                        let existing = self
                            .kv_store
                            .kv_read(
                                super::util::SQUARE_KV_PRIMARY_NAMESPACE,
                                super::util::SQUARE_KV_SECONDARY_NAMESPACE,
                                &key,
                            )
                            .await?;
            ...
            if !payment_hashes_to_store.is_empty() {
                ...
                // Calculate expiry time for all payments in this batch
                ...
                for (payment_hash, payment_id) in &payment_hashes_to_store {
                    ...
                    // Create PaymentData with expiry timestamp
                    let payment_data = PaymentData::new(expires_at, payment_id.clone());
        ...
        match self.remove_expired_payments().await {
            Ok(count) => {
                if count > 0 {
                    tracing::debug!(count, "Removed expired Square payments");
                }
            }
            Err(error) => {
                tracing::warn!(
                    error = ?error,
                    "Failed to remove expired Square payments"
                );
            }
        }
